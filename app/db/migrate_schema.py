...
def migrate_database():
    ...
    try:
        ...
        if 'stock_status' not in columns:
            ...
            ''')
            logger.info("Added stock_status column to stock_quotes table")
        ...
        if 'stock_status' not in pred_columns:
            ...
            ''')
            logger.info("Added stock_status column to predictions table")
        ...
        if 'download_attempts' not in columns:
            ...
            ''')
            logger.info("Added download_attempts column to stock_quotes table")
        
        if 'last_download_attempt' not in columns:
            cursor.execute('''
                ALTER TABLE stock_quotes ADD COLUMN last_download_attempt TEXT
            ''')
            logger.info("Added last_download_attempt column to stock_quotes table")
