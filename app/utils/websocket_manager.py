...
class WebSocketManager:
    ...
    def emit_prediction_update(self, prediction_data: Dict[str, Any]):
        ...
        if self.socketio:
            self.socketio.emit('prediction_update', prediction_data, broadcast=True)
    ...
    def emit_watchlist_update(self, watchlist_data: Dict[str, Any]):
        ...
        if self.socketio:
            self.socketio.emit('watchlist_update', watchlist_data, broadcast=True)
    ...
    def emit_stock_price_update(self, price_data: Dict[str, Any]):
        ...
        if self.socketio:
            self.socketio.emit('stock_price_update', price_data, broadcast=True)
    ...
    def emit_background_worker_status(self, status_data: Dict[str, Any]):
        ...
        if self.socketio:
            self.socketio.emit('background_worker_status', status_data, broadcast=True)
    ...
    def emit_prediction_progress(self, progress_data: Dict[str, Any]):
        ...
        if self.socketio:
            self.socketio.emit('prediction_progress', progress_data, broadcast=True)
